...
#include "AnimNotifyState/AnimNotify/AnimNotifyApplyDamageNormal.h"
...
UAnimNotifyApplyDamageNormal::UAnimNotifyApplyDamageNormal()
{
}

void UAnimNotifyApplyDamageNormal::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
...
void UAnimNotifyApplyDamageNormal::SpawnEffectAndApplyDamge(UWorld* World, const TArray<ATBC_CharacterBase*> Targets,
	UNiagaraSystem* Vfx, int32 Index, ATBC_CharacterBase* SkillOwner, float Interval)
{
	...
	World->GetTimerManager().SetTimer(
		NextAttack,
		FTimerDelegate::CreateUObject(this, &UAnimNotifyApplyDamageNormal::SpawnEffectAndApplyDamge, World, Targets, Vfx, Index + 1,  SkillOwner, Interval),
		Interval,
		false
		);
}
